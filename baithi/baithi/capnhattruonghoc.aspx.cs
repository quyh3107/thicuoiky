...
namespace baithi
{
    public partial class capnhattruonghoc : System.Web.UI.Page
    {
        ...
        {
            string chuoiSQL;
            chuoiSQL = "SELECT * FROM tbl_truong WHERE MaTruong = '" + txtMaTruong.Text + " '";
            if (Check(chuoiSQL))
            {
                Label1.Text = "Thông tin đã tồn tại";
                return;
            }
            if (txtMaTruong.Text != chuoiSQL)
            {
                string nhap = "INSERT INTO tbl_truong values('" + txtMaTruong.Text + "',N'" + txtTenTruong.Text + "')";
                ThucThi(nhap);
                HienThi();
            }
            ThucThi(chuoiSQL);
